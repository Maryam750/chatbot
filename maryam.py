...
app = Flask(__name__)
app.secret_key = "Maryam"

openai.api_key = "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"

driver = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "maryo750"))
...
myBot.respond("set name Maryam", "0")
myBot.setPredicate("Maryam", "Maryam", sessionID="0")
...
user_credentials = {
    "Maryam": {"password": "001", "name": "Maryam"},
    "xyz": {"password": "003", "name": "xyz"},
    "Alpha": {"password": "password", "name": "Alpha"},
}
...
def get_response():
    ...
    if request.method == "POST":
        ...
        if user_input.lower() in ["who are you?", "what is your name?","your name"]:
            return f"I am Nugget, your chatbot assistant!"
        if user_input.lower() in [
            ...
            return "Maryam created me. She is my Botmaster!"
        ...
        if "FETCH GPT3 RESPONSE" in response:
            ...
            with driver.session() as session:
                ...
                session.write_transaction(
                    create_username_node, "User", {"name": "Maryam"}
                )
                session.write_transaction(
                    create_username_node, "Topic", {"name": asked_about}
                )
                session.write_transaction(
                    create_relation, "Maryam", "ASKED_ABOUT", asked_about
                )
                session.commit()
